      ...
      <div className="max-w-3xl mx-auto bg-white/90 dark:bg-slate-900/80 p-6 rounded-2xl shadow-xl backdrop-blur">
        ...
        <div className="space-y-3">
          {current.options.map((opt, idx) => {
            ...
            const isWrong = feedback === "wrong" && isSelected && idx !== current.correctIndex;
        ...
        <div className="space-y-4 max-h-[380px] overflow-y-auto">
          {review.map((item, idx) => (
            ...
            >
              ...
              <p
                className={`text-sm ${
                  item.userAnswer === item.correctAnswer ? "text-emerald-600" : "text-rose-600"
                }`}
              >
                ✦ Bạn chọn:{" "}
                {item.userAnswer === null ? "Không trả lời" : item.userAnswer + 1}
              </p>
